...
- `GET /api/np/settlements?q=` and `GET /api/np/warehouses?city=&method=branch|postomat&q=` are the Нова Пошта directory proxy (ua-checkout U4, not consumed by any UI until U5): the key stays server-side, per-city warehouse lists are page-merged and cached ~24h (settlement searches ~5min, failures 30s), responses are minimized to `{ref,label,region?}` / `{number,label}` and capped, and EVERY failure mode — no key, timeout, upstream error, `success:false`, a container that decodes to zero rows — collapses to one 503. Filtering and row caps live here, not in the client (D-7); `dynamic = "force-dynamic"` on both routes.
