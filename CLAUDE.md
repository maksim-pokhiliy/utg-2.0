...
- `yarn e2e` — blank-env `yarn build` + Playwright against `next start`: the script and `webServer.env` both blank `PLACE_ORDER_URL`/`EXCHANGE_RATE_API_*`/`NOVA_POSHTA_API_KEY`, so checkout's real 503 is the deterministic error fixture and the suite can never hit the live relay or the live carrier directory (nor flake on live rates). Every spec registers its own rate-limit identity in `e2e/support/app.ts` (the limiter is live in the battery since the directory routes landed; an unregistered spec silently shares the socket-derived bucket)
...
The app boots and builds with zero env vars (the exchange-rates fetch is guarded → prices fall back to UAH for both locales). `.env.example` documents the five optional keys: `EXCHANGE_RATE_API_URL`, `EXCHANGE_RATE_API_KEY` (USD conversion for `en`), `PLACE_ORDER_URL` (order relay — checkout returns 503 without it), `NOVA_POSHTA_API_KEY` (Нова Пошта directory — the proxy routes answer 503 without it, which is the signal the uk checkout will use to fall back to free-text), `ORDER_RELAY_SECRET` (sent as `x-relay-secret`; the relay 401s callers without it — LIVE in prod since 2026-08-06, so the two projects' values must stay in sync).
...
- `POST /api/place_order` proxies the checkout payload to the external `PLACE_ORDER_URL` relay, forwarding upstream status; its 500 body carries no internal details. It presents `x-relay-secret` when `ORDER_RELAY_SECRET` is set (printable-ASCII guard — an unsendable value answers 503 rather than letting `fetch` throw the secret into the logs), strips trailing slashes off the relay origin (Vercel 308-normalizes `//path` before app code, which under the next rule would be a total outage), and sets `redirect: "error"` — a followed redirect hands the secret AND the customer's whole order to hop two, which platform header-stripping does not cover. The payload field shape (`first_name`…`cart[{title,quantity,productUrl,…}]`, `locale`, `total`, `currency` per D-12) is a sacred contract with the bot (the bot reads `currency` in a separate bot-repo follow-up).
- `GET /api/np/settlements?q=` and `GET /api/np/warehouses?city=&method=branch|postomat&q=` are the Нова Пошта directory proxy. The key stays server-side and **the warehouse SEARCH is delegated to the carrier** via `FindByString` (D-14): one upstream page per query, no page-merge, no whole-city corpus — Kyiv reports over 12 000 points, so the old ~24h merged city cache was never completable. The warehouse cache is keyed `(city, query)` ~5min — `method` never entered the upstream body, so ONE cached page holds both chips' answers and the category split happens on read (D-18); settlement searches ~5min, failures 30s. The cache key is length-prefixed, hence injective by construction, and `city` is shape-guarded (`[0-9a-f-]`, ≤64) to a 400 before it can reach the carrier. Responses are minimized to `{ref,label,region?,warehouseCount,isCourierAllowed}` / `{number,label}` and capped; carrier-derived strings are read through one `readString` that strips `\p{Cf}`, so bidi controls can never reorder a branch number in front of a buyer. EVERY failure mode — no key, timeout, upstream error, `success:false`, a container that decodes to zero rows, and a page where NO category is recognised — collapses to one 503; a page we recognise but do not serve is an honest 200-empty. **What we delegate is the search, never the policy** (D-7 stands): the category filter, the row cap, the refusal of points the carrier marks unselectable and every failure decision are ours. `dynamic = "force-dynamic"` on both routes.
- `src/app/api/np/guard.ts` bounds what the carrier layer may spend, and `refusal.ts` keeps our own verdicts out of the cache. A damper stops calling Нова Пошта after **three consecutive** distress signals for 30s, and a fan-out bound caps concurrent upstream calls at 12, refusing over-cap calls immediately rather than queueing them behind a 7s budget. **Distress is classified by the carrier's `errorCode`, never by English prose** (D-17): transport failures, any 5xx, HTTP 429 and `success:false` carrying `20000401501` arm it; every other `success:false` — `20000900768` "City not found" included — and every non-429 4xx is a per-key verdict that answers 503 without arming anything and RESETS the consecutive count. Never widen this to "unknown code ⇒ outage": failing to arm costs bounded hammering, arming wrongly blacks out the directory for every buyer. **The negative cache stores what the CARRIER said about a key, never what WE decided about ourselves** (D-16) — an over-cap refusal, a damped refusal and the no-key path are a distinct `CARRIER_REFUSED` symbol the cache is structurally unable to store (the compiler refuses it; deleting the guard is a compile error).
- `src/app/api/rate-limit.ts` is the shared per-IP in-memory limiter: `place_order` at 5/60s, both directory routes on one shared 60/60s bucket, 429 + Retry-After. It runs before body parsing and fails OPEN when no client identity exists — a false 429 costs a real volunteer order (see UAC-11 for the identity-forgery caveat).
